...
Maintainers create the update kit with `./scripts/create_update_archive.sh`. Pushing a matching version tag such as `v1.10.0` runs `.github/workflows/release-update.yml`, tests the source, and publishes the update assets. The updater remains compatible with the older five-argument helper call used by RayPlacement 1.7.x. The full Desktop installer remains the recovery path if a local model or build tool is missing.
