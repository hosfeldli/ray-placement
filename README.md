...
1. Double-click `Install RayPlacement.command`. It verifies the bundled Qwen and Local Whisper assets, repairs missing files from pinned official downloads when needed, creates a stable local signing identity with your consent, builds and signs for this Mac, checks the resulting bundle, and installs the app and bundled extensions. A source ZIP may download about 2.3 GB on first install; later updates reuse both verified local models.
...
RayPlacement currently targets Apple-silicon Macs running macOS 13 or later. Install Swift 6 from Xcode 16 Command Line Tools or newer, then double-click `Install RayPlacement.command`. Git LFS is recommended for a clone but is no longer required for a downloaded source ZIP: the installer detects incomplete assets and retrieves the exact pinned Qwen model, llama.cpp runtime, and Whisper small.en model from their official repositories. Every local, reconstructed, or downloaded asset is checked against its expected SHA-256 before use. The installer does not reuse or copy a signing key from another computer. It explains the local trust change, asks before making it, creates a private key that stays on that Mac, rebuilds RayPlacement locally, verifies the bundled models, and installs to `~/Applications/RayPlacement.app`.
...
- Long-meeting note dictation with selectable Local Whisper or Apple on-device recognition, two-hour rolling recording at every performance level, and recoverable segmented audio
...
- Per-task local model assignment across Alibaba Qwen, Mistral AI, Google Gemma, and Microsoft Phi possibilities; official downloads/imports are SHA-256 verified and the bundled Qwen3 1.7B remains the safe fallback
...
Run **RayPlacement Notes** from the launcher, choose **Notes…** in the menu bar, or use the configurable **Open Notes** shortcut in **Settings → General**. **Quick Note Sidebar** has its own configurable global shortcut and pins the most recent note to either screen edge beside the current app; optional global Dock Left and Dock Right shortcuts move it instantly. The compact header can move the dock, restore the full workspace, hide the note list, or enter a screen-filling focus mode; the prior size and position are restored when focus mode ends. Notes are saved as local Markdown data in `~/Library/Application Support/RayPlacement/notes.json`. There is no separate source/preview split: headings, emphasis, links, lists, tasks, quotes, inline code, fenced code blocks, and GitHub-style tables are formatted directly inside the editable document. Markdown punctuation collapses out of the normal reading view while the portable source remains intact underneath. Paste a table copied from a spreadsheet, website, Markdown, TSV, or CSV source and Notes converts it into an editable native table; pasting a grid into an existing table fills from the active cell and grows the table as needed. Every table can be named in its compact header and sorted ascending or descending by any column. Use the format menu to insert headings, quotes, dividers, or tables. Command-B, Command-I, and Command-K format the current selection, and Return continues lists and task lists. Formatting is debounced so typing stays responsive.

Choose **Summarize** in a note to run the selected local model on demand. Long notes are divided into bounded sections and reduced into a final Markdown summary; only one local model job can run at a time. The result opens for review and can be copied or inserted at the top of the original note. It uses **Settings → Performance → Writing**, follows the selected Auto/Metal/CPU compute mode, sends no note content over the network, and exits when finished or cancelled.

Press **Dictate**, speak normally, then choose **Stop**. The configurable dictation shortcut opens the most recently edited note and starts recording; press it again to stop and finish transcription. Closing the Notes window does not stop the recording: a compact speech-level panel stays centered above the Dock with elapsed time, input feedback, the destination note, and Stop/Cancel controls. Pressing the Notes shortcut again toggles the Notes window without stopping an active recording.

**Local Whisper · Recommended** records lossless 16 kHz mono PCM in durable one-minute files and runs the bundled CPU-only Whisper small.en model. It is more tolerant of meeting-room distance and needs only Microphone permission. By default, transcription waits for Stop to avoid background load; enable **Transcribe completed segments while recording** in **Settings → Performance** to process each finished minute while the meeting continues. **Apple Speech** remains selectable for a lighter system-native path and uses 45-second files. Apple partial results are preserved if recognition ends early. Neither engine has a transcription timeout, and recording can continue for two hours at every performance level. If microphone input unexpectedly ends, RayPlacement immediately starts a fresh segment instead of ending the meeting. Failed sessions are moved intact to `~/Library/Application Support/RayPlacement/Failed Dictations` for Retry. Successful audio is deleted. One hour is about 115 MB; the two-hour ceiling is about 230 MB.
...
| --- | --- | --- | --- |
| Eco | 1 background CPU thread; 90-second limit; 256-token summaries | 2-hour recording; 1 transcription thread; no timeout | background priority; cooperative 1-thread limit; 60-second hard timeout |
| Balanced | 2 utility CPU threads; 120-second limit; 384-token summaries | 2-hour recording; 2 transcription threads; no timeout | utility priority; cooperative 2-thread limit; 180-second hard timeout |
| High | 4 foreground CPU threads; 180-second limit; 512-token summaries | 2-hour recording; 4 transcription threads; no timeout | foreground priority; cooperative 4-thread limit; 600-second hard timeout |
| Turbo | Up to 6 foreground CPU threads; 300-second limit; 768-token summaries | 2-hour recording; up to 6 transcription threads; no timeout | foreground priority; up to 6 cooperative threads; 1,200-second hard timeout |
| Maximum | Up to 12 CPU threads, never more than the Mac exposes; 600-second limit; 1,024-token summaries | 2-hour recording; up to 12 transcription threads; no timeout | foreground priority; up to 12 cooperative threads; 3,600-second hard timeout |
| Unbounded | Every logical CPU exposed by macOS; no wall-clock timeout; 2,048-token summaries | 2-hour recording; every logical CPU; no timeout | highest process priority; every logical CPU supplied cooperatively; no RayPlacement wall-clock timeout |

Local text models launch only for a requested grammar check, summary, or formatter proposal and exit when the job completes or times out. **Auto** offloads supported layers to the Apple GPU and retries on CPU if Metal cannot start; **Apple GPU** forces Metal, and **CPU only** keeps inference off the GPU. Memory depends on the selected model, from about 639 MB to more than 4 GB on disk. Local Whisper remains CPU-only, follows the Dictation thread limit, and exits when the segment queue is complete; its small.en model is about 466 MB. The optional during-recording mode starts it only after a complete one-minute file is safely closed. Dictation is never stopped by a model deadline. Apple controls the internal scheduling of its on-device recognizer. RayPlacement enforces extension priority, output size, and wall-clock time except when the user explicitly chooses Unbounded; it also supplies common AI thread-limit environment variables, but an untrusted third-party executable can ignore those cooperative variables.
...
The included [Extensions/writing-tools](Extensions/writing-tools) extension adds **Paste as Plain Text** (`Control-Option-V`) and **Check Spelling & Grammar** (`Control-Option-G`). Grammar correction goes directly to the selected local AI model and falls back to the bundled Qwen3 1.7B Q8 model if an optional model is unavailable. A deterministic first pass is independently audited by a second AI pass; tagged-response validation and one bounded retry prevent partial explanations or malformed output from reaching the review. No Harper, NSSpellChecker, or rule-based grammar pass changes the text. The same settings page includes persistent correction instructions for protected names and terms, capitalization, dialect, tone, and preferred style.
...
The repository contains the provider runtimes and pinned asset metadata. To reproduce all historical provider assets from pinned upstream releases, run `./scripts/fetch_vendor_assets.sh`. The shipping app packages Qwen with llama.cpp and Whisper small.en with whisper.cpp. `./scripts/assemble_qwen_model.sh` and `./scripts/assemble_whisper_model.sh` verify local assets and repair missing models from their official repositories. The fully local models add about 2.2 GB to the app.
...
Maintainers create the update kit with `./scripts/create_update_archive.sh`. Pushing a matching version tag such as `v1.11.0` runs `.github/workflows/release-update.yml`, tests the source, and publishes the update assets. The updater remains compatible with the older five-argument helper call used by RayPlacement 1.7.x. The full Desktop installer remains the recovery path if a local model or build tool is missing.
...
- `Sources/RayPlacement/NoteDictationService.swift` — explicit record-then-transcribe dictation with Local Whisper and Apple engines
- `Extensions` — bundled Writing Tools, Endpoint Tester, Emoji Picker, VS Code Directories, Productivity Tools, and Document Formatter extensions
