...
QWEN_PARTS_DIRECTORY="$QWEN_DIRECTORY/ModelParts"
...
mkdir -p "$DOWNLOAD_DIRECTORY" "$HARPER_DIRECTORY" "$MODEL_DIRECTORY" "$QWEN_DIRECTORY" "$QWEN_PARTS_DIRECTORY"
...
rm -f "$QWEN_PARTS_DIRECTORY"/Qwen3-1.7B-Q8_0.gguf.part-*
split -b 300m -a 2 "$QWEN_MODEL" "$QWEN_PARTS_DIRECTORY/Qwen3-1.7B-Q8_0.gguf.part-"
(
  cd "$QWEN_PARTS_DIRECTORY"
  shasum -a 256 Qwen3-1.7B-Q8_0.gguf.part-* > SHA256SUMS
)
